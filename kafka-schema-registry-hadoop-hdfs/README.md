...
- Ограничения и рекомендации: для рабочей нагрузки нужно выделить больше ресурсов Docker (RAM/CPU), на проде стоит добавить мониторинг (Prometheus/Grafana), настройки retention/cleanup для Kafka и безопасный доступ (аутентификация, TLS) для Schema Registry и брокеров; также рекомендуется обработка ошибок при записи в HDFS (retry, dead‑letter) и управление смещениями консьюмера (commit semantics).  
